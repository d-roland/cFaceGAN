...
desc += '-preset-v2-1gpu'; num_gpus = 1; sched.minibatch_base = 4; sched.minibatch_dict = {4: 128, 8: 128, 16: 128, 32: 64, 64: 32, 128: 16, 256: 8, 512: 4}; sched.G_lrate_dict = {1024: 0.0015}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 6009
...
#train = EasyDict(func='util_scripts.generate_fake_images_labels', run_id=16, num_pngs=1, grid_size=[7,4]); num_gpus = 1; desc = 'fake-images-' + str(train.run_id)
...
train = EasyDict(func='util_scripts.generate_interpolation_video', run_id=0, grid_size=[2,2], duration_sec=60.0, smoothing_sec=1.0); num_gpus = 1; desc = 'interpolation-video-' + str(train.run_id)
#train = EasyDict(func='util_scripts.generate_training_video', run_id=16, duration_sec=20.0); num_gpus = 1; desc = 'training-video-' + str(train.run_id)

#train = EasyDict(func='util_scripts.evaluate_metrics_multi', run_ids=[16], log='metric-all.txt', metrics=['swd', 'fid', 'is', 'msssim'], num_images=20000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_ids); label_files=['proGANin-rxx.labels.16']
